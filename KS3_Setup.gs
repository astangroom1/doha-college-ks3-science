...
function buildP1StudentEmail_(student, wrongQs, tasks, score, pct, classInfo, config, mode, subToken) {
  ...
  // Fix 3: sign-off always uses full name (Mr Dettman), never first name
  var teacherName  = classInfo.leadName;
  var teacherEmail = classInfo.leadEmail;

  // Submission URL — shown in both LIVE and TEST mode if web app is configured
  var submitUrl = (subToken && config.webAppUrl)
    ? config.webAppUrl + '?token=' + subToken
    : null;

  // In TEST mode, generate a placeholder URL so the button is visible for testing
  if (!submitUrl && config.webAppUrl && mode === 'TEST') {
    submitUrl = config.webAppUrl + '?token=TEST_TOKEN_PREVIEW';
  }

  // ── 1. SCORE BOX — tier badge centred between columns ────────
  // Third row spans both columns so badge sits between them
  var tierCentreRow = tierInfo
    ? '<tr><td colspan="2" style="padding:0 16px 10px;text-align:center;">' +
      '<div style="display:inline-block;background:' + tierInfo.bg + ';color:' + tierInfo.fc +
      ';border-radius:20px;padding:3px 16px;font-size:11px;font-weight:bold;letter-spacing:0.3px;">' +
      tierInfo.label + '</div></td></tr>'
    : '<tr><td colspan="2" style="padding:0 16px 8px;text-align:center;">' +
  ...
  var scoreBox =
    ...
    '<tr>' +
    '<td style="width:50%;padding:12px 16px 6px;text-align:center;border-right:1px solid #c5cae9;">' +
    ...
    '</td>' +
    '<td style="width:50%;padding:12px 16px 6px;text-align:center;">' +
  ...
  // ── 2. INSTRUCTIONS — updated to include submission step ─────
  var submitSection = submitUrl
    ? '<div style="margin:10px 0 0;padding:14px 18px;background:#e0f2f1;border-radius:8px;">' +
      '<h3 style="margin:0 0 6px;font-size:13px;color:#004d40;font-weight:bold;">📷 Step 2 — Submit Your Work</h3>' +
      '<p style="margin:0 0 10px;font-size:13px;color:#00695c;line-height:1.6;">' +
      'When you have completed <strong>all</strong> tasks in your exercise book, take a clear photo of your work and submit it using the button below. ' +
      'Your teacher will review your submission — once approved, your model answers will be sent automatically so you can self-assess.' +
      '</p>' +
      '<table style="width:100%;border-collapse:collapse;margin-bottom:12px;">' +
      '<tr><td style="padding:3px 8px 3px 0;font-size:12px;color:#004d40;width:24px;">1.</td><td style="padding:3px 0;font-size:12px;color:#00695c;">Complete all tasks in your exercise book</td></tr>' +
      '<tr><td style="padding:3px 8px 3px 0;font-size:12px;color:#004d40;">2.</td><td style="padding:3px 0;font-size:12px;color:#00695c;">Tap the button below on your phone or device</td></tr>' +
      '<tr><td style="padding:3px 8px 3px 0;font-size:12px;color:#004d40;">3.</td><td style="padding:3px 0;font-size:12px;color:#00695c;">Take or upload a clear, legible photo of your work</td></tr>' +
      '<tr><td style="padding:3px 8px 3px 0;font-size:12px;color:#004d40;">4.</td><td style="padding:3px 0;font-size:12px;color:#00695c;">Tap Submit — wait for the confirmation before closing</td></tr>' +
      '<tr><td style="padding:3px 8px 3px 0;font-size:12px;color:#004d40;">5.</td><td style="padding:3px 0;font-size:12px;color:#00695c;">Your teacher reviews it and sends your model answers</td></tr>' +
      '</table>' +
      '<div style="text-align:center;">' +
      '<a href="' + submitUrl + '" style="display:inline-block;background:#1a237e;color:#fff;padding:14px 32px;border-radius:8px;font-size:15px;font-weight:bold;text-decoration:none;letter-spacing:0.3px;">📷 Submit My Work</a>' +
      '</div>' +
      ...
      '<p style="margin:10px 0 0;font-size:10px;color:#80cbc4;text-align:center;">This link is personal to you · Valid for 7 days · 3 attempts allowed</p>' +
      '</div>'
    : '';

  var instructions =
    '<div style="margin:18px 0 0;padding:14px 18px;background:#e8eaf6;border-radius:8px;">' +
    '<h3 style="margin:0 0 6px;font-size:13px;color:#1a237e;font-weight:bold;">Step 1 — Complete Your Tasks</h3>' +
    '<p style="margin:0;font-size:13px;color:#3949ab;line-height:1.6;">' +
  ...
  // ── 3. TASK SUMMARY — bar chart + AO donut ────────────────────
  var questionSummary = '';
  if (wrongQs.length > 0) {
    ...
    // AO breakdown — segmented bar (Gmail-safe, no canvas/SVG needed)
    ...
    var aoColours = { 'AO1': '#1a237e', 'AO2': '#5c6bc0', 'AO3': '#9fa8da' };
